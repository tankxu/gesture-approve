...
| **摄像头 capture session** | USB 采集卡打嗝 / 睡眠挂起 → `isRunning` 仍 true 却不吐帧 → 刘海黑屏 | ①审批期帧看门狗 1.5s 无帧重建;②唤醒/解锁主动 `invalidate()` 下次重配;③runtime error/中断结束也触发 | `FrameSource.swift`,详见 [camera-resilience.md](./camera-resilience.md) |
...
| `didWake` | `asleep = false`;`server.restart()`(复活监听);`controller.handleSystemWake()`(复位摄像头) |
| `screenIsLocked` | 无动作(锁屏状态靠 `screenLocked` 实时查,不缓存) |
| `screenIsUnlocked` | `server.restart()`;`controller.handleSystemWake()` |
