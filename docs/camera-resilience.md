...
`VideoInputs.currentID()` 在"选定设备不在 `available()` 列表里"时**会回退到内置摄像头**——这对 UI 选择是对的，但对"唤醒瞬间"是灾难：USB 采集卡刚唤醒还没枚举完，就被判成"不存在"而回退成 FaceTime，于是预热/审批全程开错设备（还识别不准）。

所以**实际打开设备一律走 `VideoInputs.savedOrDefaultID()`**（坚持用户保存的选择，不回退），且 `configureIfNeeded` 在指定了 `deviceUniqueID` 时**严格只用该设备，找不到就当"未就绪"等它枚举，绝不 fallback 到别的摄像头**。`requestApproval` 与 `handleSystemWake` 都用同一个 `savedOrDefaultID()`，否则两者用不同 id 会互相重建打架。代价：设备被永久拔掉时会黑屏（而非偷偷用内置），这是有意为之——用户选了特定采集卡就该用它，用错摄像头比黑屏更糟。
...
- **用对设备**：实际打开设备走 `savedOrDefaultID()`（不回退），`configureIfNeeded` 严格用选定设备不 fallback。详见上节"用对设备"。
...
- 唤醒后第一次审批，应只见 `camera 使用 <你选的设备>`，**绝不该出现 `FaceTime` 之类没选的设备**（出现即"用错设备"回归）。
- `camera 选定设备未就绪(等待枚举)` = USB 还在重新枚举，看门狗在等它回来，属正常过渡。
